...
export default function PreloadPage() {
    const crud = useCrud<StockRecord>({ endpoint: "asn/preloadstock/", queryKey: "preload" });
    return (
        <div className="space-y-6">
            <div><h1 className="text-2xl font-bold">Pre-Load Stock</h1><p className="text-muted-foreground">Stock ready for loading</p></div>
            <CrudTable title="Pre-Load" columns={columns} data={crud.data} totalCount={crud.totalCount} page={crud.page} totalPages={crud.totalPages} isLoading={crud.isLoading} onPageChange={crud.goToPage} onSearch={crud.setSearch} onRefresh={() => crud.refetch()} />
        </div>
    );
}
